jQuery(function($) {
	var supportedTypes = ['video/mp4', 'video/webm', 'video/ogv'];
	
	// Cloudinary Video
	$('.field.type-cloudinaryvideo').each(function() {
		...
		var removeNewVideo = function() {
			$el.find('.image-preview.new').remove();
		};

		var removeSelectVideo = function() {
		...
		var checkExistingVideo = function() {
			if (data.fieldValue) {
				// Show it
				$imagePreview.show();
		...
		$upload.change(function(e) {
			...
			var renderPlaceholder = function() {
				...
				$deleteBtn.hide();
				$cancelBtn.show();
				$uploadBtn.text(imageSelected ? 'Change Video' : 'Upload Video');
				// Preview
				removeNewVideo();
				// Render HTML
				...
				if (window.FileReader) {
					...
					for (var i = 0, f; f = files[i]; i++) {
						if (!_.contains(supportedTypes, f.type)) {
							$upload.val('');
							alert("Unsupported file type. Supported formats are: MP4, OGV, WEBM");
							continue;
						}
		...
		$deleteBtn.click(function(e) {
			...
			// Messages
			$deleteQueued.show();
			$deleteQueued.find('.alert').html('Video '+ ( action == 'delete' ? 'deleted' : 'removed') + ' - save to confirm');
			// Redraw
			$(window).trigger('redraw');
		});
		...
		$select2Input.each(function(i, el) {
			...
			el.select2({
				placeholder: 'Search for a video from Cloudinary ...',
				allowClear: true,
				multiple: false,
				width: "60%",
				loadMorePadding: 100,
			...
			el.on('change', function(e) {
				var select2data, imageWrapper, $newVideo;
				...
				imageWrapper = '<div class="image-preview select">' +
					...
					'</div>';
				$newVideo = $('<img class="placeholder" />').attr('src', select2data.url);

				$newVideo.on('load', function() {
