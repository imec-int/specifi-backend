...
/**
 * CloudinaryVideo FieldType Constructor
 * @extends Field
 * @api public
 */

function cloudinaryvideo(list, path, options) {
	...
	if (options.initial) {
		throw new Error(
			...
			'CloudinaryVideo fields (' + list.key + '.' + path + ') do not currently support being used as initial fields.\n'
		);
	}

	options.templateDir = fspath.normalize( __dirname + '../../../templates/fields/' + this.constructor.name);

	cloudinaryvideo.super_.call(this, list, path, options);

	// validate cloudinary config
	if (!keystone.get('cloudinary config')) {
		throw new Error(
			'Invalid Configuration\n\n' +
			'CloudinaryVideo fields (' + list.key + '.' + this.path + ') require the "cloudinary config" option to be set.\n\n' +
...
util.inherits(cloudinaryvideo, super_);
...
cloudinaryvideo.prototype.addToSchema = function() {
	...
	var schemaMethods = {
		...
		tag: function(options) {
			return exists(this) ? cloudinary.video(this.get(field.path), options) : '';
		},
...
cloudinaryvideo.prototype.format = function(item) {
...
cloudinaryvideo.prototype.isModified = function(item) {
...
cloudinaryvideo.prototype.validateInput = function(data) {
	// TODO - how should video field input be validated?
	return true;
};
...
cloudinaryvideo.prototype.updateItem = function(item, data) {
	var paths = this.paths;
...
cloudinaryvideo.prototype.getRequestHandler = function(item, req, paths, callback) {
	...
	return function() {
		...
		if (req.body && req.body[paths.select]) {
			...
			if (field.options.autoCleanup && item.get(field.paths.exists)) {
				// capture video delete promise
				videoDelete = field.apply(item, 'delete');
			}
			...
			// upload immediately if video is not being delete
			if (typeof videoDelete === 'undefined') {
				field.apply(item, 'upload', req.files[paths.upload].path, uploadOptions).onFulfill(uploadComplete);
			} else {
				// otherwise wait until video is deleted before uploading
				// this avoids problems when deleting/uploading videos with the same public_id (issue #598)
				videoDelete.onFulfill(function(result) {
					if (result.error) {
						callback(result.error);
					} else {
						field.apply(item, 'upload', req.files[paths.upload].path, uploadOptions).onFulfill(uploadComplete);
...
cloudinaryvideo.prototype.handleRequest = function(item, req, paths, callback) {
...
exports = module.exports = cloudinaryvideo;
