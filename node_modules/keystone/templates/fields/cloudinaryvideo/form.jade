- var hasVideo = item.get(field.paths.exists)
.field.type-cloudinaryvideo(data-field-type=field.type, data-field-path=field.path, data-field-collapse=field.collapse ? 'true' : false, data-field-depends-on=field.dependsOn, data-field-noedit=field.noedit ? 'true' : 'false', data-field-value=hasVideo ? 'true' : 'false')
	label.field-label= field.label
	input(type='file', name=field.paths.upload).field-upload
	input(type='hidden', name=field.paths.action, value='').field-action
	.field-ui(class=hasVideo ? 'has-image' : false)
		if field.noedit
			if hasVideo
				.image-preview
			...
			.image-container
				...
				.image-details
					...
					.upload-queued.pull-left
						.alert.alert-success Video selected - save to upload
					.select-queued.pull-left
						.alert.alert-success Video selected from Cloudinary
					.delete-queued.pull-left
						.alert.alert-danger
			.image-toolbar
				div.pull-left
					button(type='button').btn.btn-default.btn-upload-image= (hasVideo ? 'Change' : 'Upload') + ' Video'
					if field.options.autoCleanup
						button(type='button', data-alt-text='Remove Video', data-default-action='delete', data-alt-action='reset', style=(hasVideo ? '' : 'display: none;')).btn.btn-link.btn-cancel.btn-delete-image Delete Video
					else
						button(type='button', data-alt-text='Delete Video', data-default-action='reset', data-alt-action='delete', style=(hasVideo ? '' : 'display: none;')).btn.btn-link.btn-cancel.btn-delete-image Remove Video
					button(type='button', style='display: none;').btn.btn-link.btn-cancel.btn-cancel-image Cancel Upload
					button(type='button', style='display: none;').btn.btn-link.btn-cancel.btn-undo-image Undo Remove
